...
namespace BankaaAPI.Controllers.API
{
    ...
    {
        BitirmeBankaEntities1 db = new BitirmeBankaEntities1();
        ...
        public IHttpActionResult PostPayService(string MerchantCode, string MerchantPassword, string CardNo, string ExpM, string ExpY, string CVV, decimal Price)
        {
            ...
            if (merchantCount != 0)
            {
                ...
                int cardControl = db.MusteriHesaplar.Count(x => x.KartNumarasi == CardNo);
                if (cardControl != 0)
                {
                    MusteriHesaplar mh = db.MusteriHesaplar.FirstOrDefault(x => x.KartNumarasi == CardNo);
        ...
        public IHttpActionResult PostPayService(PayServiceModel model)
        {
            ...
            if (merchantCount != 0)
            {
                ...
                int cardControl = db.MusteriHesaplar.Count(x => x.KartNumarasi == model.CardNo);
                if (cardControl != 0)
                {
                    MusteriHesaplar mh = db.MusteriHesaplar.FirstOrDefault(x => x.KartNumarasi == model.CardNo);
